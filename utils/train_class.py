...
def get_dataloaders(h5_path, edge_index, stats, batch_size, val_split=0.2, limit=None, num_workers=2, target=None):
...
def train_epoch(model, loader, loss_fn, optimizer, device):
    ...
    for batch in loader:
        ...
        # logger.info(f"preds: {preds.shape}, targets: {batch.y.shape}")
        # logger.info(f"preds: {preds}, targets: {batch.y}")
        loss.backward()
        optimizer.step()
...
def main():
    ...
    # --- Data ---
    train_loader, val_loader = get_dataloaders(h5_path, 
                                               edge_index, 
                                               stats, 
    ...
    plot_losses(train_losses, val_losses, save_path=os.path.join(output_dir, "loss_curve_regressor.png"))
