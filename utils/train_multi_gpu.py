...
def train_epoch(model, loader, loss_fn, acc, optimizer, device, device_type, scaler, max_grad_norm=1.0):
    model.train()
    total_loss = 0
    all_preds = []
    all_labels = []
...
def validate(model, loader, loss_fn, acc, device, device_type):
    model.eval()
    total_loss = 0
    all_preds = []
    all_labels = []
...
def plot_accuracy(train_accs, val_accs, save_path="plots/accuracy_curve.png"):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
    plt.figure(figsize=(8,6))
...
def main():
    ...
    compute_acc = Accuracy(task='multiclass', num_classes=5 if class_type=='5-label' else 3)
    ...
    for epoch in range(epochs):
        ...
        train_loss, train_acc = train_epoch(model, train_loader, loss_fn, compute_acc, optimizer, device, device_type, scaler)
        val_loss, val_acc = validate(model, val_loader, loss_fn, compute_acc, device, device_type)      
        ...
        if get_rank() == 0:
            logger.info(f"[Epoch {epoch+1}] Train: {train_loss:.4f} | Val: {val_loss:.4f} | Train Acc: {train_acc:.4f} | Val Acc: {val_acc:.4f} | Time: {epoch_time:.1f}s")
    ...
    if get_rank() == 0:
        ...
        logger.info(f"[Post] Plotting Accuracies")
        plot_accuracy(train_accuracies, val_accuracies, save_path=os.path.join(output_dir, "accuracy_curve_egnn.png"))
